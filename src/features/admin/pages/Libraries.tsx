...
export function Libraries() {
	...
	const status = (params.get("status") as "all" | LibraryStatus) || "all";
		...
		<div className="mx-auto max-w-7xl space-y-6">
			...
			<div className="rounded-2xl border border-slate-200 bg-white shadow-sm shadow-slate-200/30">
					...
					<div className="flex items-center gap-2 overflow-x-auto">
						{statuses.map((s) => (
							<button
								...
								className={`whitespace-nowrap rounded-lg px-3 py-2 text-xs font-bold capitalize ${status === s ? "bg-slate-900 text-white" : "text-slate-500 hover:bg-slate-100"}`}>
								{s}
							</button>
						))}
				...
				<div className="flex items-center justify-between border-t border-slate-100 px-5 py-3 text-xs text-slate-400">
					<span>
						Showing {filtered.length} of {data.length} libraries
					</span>
