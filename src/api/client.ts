...
export async function reviewLibrary(id: string, status: Exclude<LibraryStatus, 'pending'>): Promise<Library> {
  const action = status === 'approved' ? 'approve' : status === 'suspended' ? 'suspend' : 'reject'
  const data = await request<{library: Library}>(`/admin/library/${action}/${id}`, { method:'POST' })
  return data.library
}
